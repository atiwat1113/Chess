...
public class Chess960Board extends NormalBoard{
	...
	public ArrayList<Point> castingPoint(Side side) {//finished
		...
		if (isRightCastling(king, side, rightRook)!=null) returnPoint.add(isRightCastling(king, side, rightRook));
		if (isLeftCastling(king, side, leftRook)!=null) returnPoint.add(isLeftCastling(king, side, leftRook));
		return returnPoint;
	}
	...
	public boolean isNull(Point oldKing, Point oldRook,Point newKing, Point newRook) {
		...
		for (int i = min(oldKing.y, oldRook.y, newKing.y, newRook.y); i<=max(oldKing.y, oldRook.y, newKing.y, newRook.y); i++) {
			Point point = new Point(oldKing.x,i);
			if (point.equals(oldRook) || point.equals(oldKing)) continue;
			if (getEntity(point)!=null) return false;
		}
		return true;
	}
	public Point isRightCastling(Entity king, Side side, Point rightRook) {
		if (rightRook == null) return null;
